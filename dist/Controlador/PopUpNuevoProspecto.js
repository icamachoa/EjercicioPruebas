...
SalesUp.Variables.ltMunicipios = function(Op){  console.log('ltMunicipios');
...
SalesUp.Variables.EnviarFormaAvanzado = function(){
	...
	$btnAceptar.attr('onclick','console.log(\'espera\');return false;');
...
function IniciaSelectizeCompanias(){
	...
	console.log('IniciaSelectizeCompanias, empresaLigada',empresaLigada);
...
function DespuesDeSeleccionarCompania(Op){
	...
	if( !_.isNaN(parseInt(Valor)) ){
		...
		if( (empresaLigada==1)&&(Valor!='')){
			console.log(SalesUp.Construye.IdUnico(),Valor, 'tiene q estar ligada');
...
function SelectizeSeleccionarEmpresa(query){ 
	console.log('SelectizeSeleccionarEmpresa',query);
...
SalesUp.Variables.UsuariosCanalizacionAutomatica = function(){
	console.log('UsuariosCanalizacionAutomatica');
}
*/
...
SalesUp.Variables.compartirElProspecto = function(Op){
  ...
  console.log('compartirElProspecto',activo);
}
