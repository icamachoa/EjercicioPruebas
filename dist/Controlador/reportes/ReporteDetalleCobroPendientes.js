var detalleCobroP = function() {
    this.reporteDetalleCobroPendiente = function(obj) {
        ...
        var vistaReporte = function(Op) {
            ...
            templateHeader     += '  <th class="centrado">Email / Telefono</th>';
            templateHeader     += '  <th class="centrado">Concepto</th>';
            templateHeader     += '  <th class="centrado">Monto</th>';
            templateHeader     += '  <th class="centrado">Comisión</th>';
           ...
           templateCuerpo     +='<td><a href="ventas-visualizar.dbsp?tkv={{tkv}}&tko={{tkoportunidad}}">{{NOMBRE}} {{APELLIDOS}}</a><br/>  {{hlpCobroPendienteEmpresa TKCOM EMPRESA}}</td>';
           templateCuerpo     +='<td>{{hlpDetalleCobroInfo TELEFONO TELEFONO2 MOVIL}}</td>';
           templateCuerpo     +='<td><div align="left">{{CONCEPTO}} - <i>Pago {{NOPARCIALIDAD}} de {{NOPARCIALIDADES}}</i></div></td>';
           templateCuerpo     +='<td><div align="tDer">{{hlp_Simbolo_Moneda MONTO MONEDA_SIMBOLO 0}}</div></td>';
           templateCuerpo     +='<td><div align="tDer">{{hlp_Simbolo_Moneda COMISION MONEDA_SIMBOLO 0}}</div></td>';
           templateCuerpo     +='<td><div align="center"> {{FECHA_COBRO}}</div></td>';
           templateCuerpo     +='<td><span align="center" class="Tip5" tip="{{NOMBRE_USUARIO}}">{{INICIALES}}</span></td>';
