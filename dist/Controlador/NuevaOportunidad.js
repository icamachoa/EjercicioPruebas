...
function Calcula_Comision(){
  ...
  if (document.frmOportunidad.monto.value>''){
    if (document.frmOportunidad.comision.value==''){
      ...
      var monto_pesos = FormatFloatCharacter(document.frmOportunidad.monto.value,DecimalSeparador);
    
      var comision_pct = FormatFloatCharacter(document.frmOportunidad.comision.value,DecimalSeparador);
    ...
    document.frmOportunidad.comision_monto.value = FormatFloatCharacterGen(tmp,DecimalSeparador,DecimalSeparador);
    
  }
  
  var $Op = $('#comision_monto'); 
  activaFormato({t:$Op},1);
  var monto = $('#comision_monto').val();
  var punto=$('#monto').val();
  $('#SUBTOTAL').val(punto);
if (monto=='NaN'){ $('#comision_monto').val(0); $('#SUBTOTAL').val(0);} 	
  if (monto=='NaN'){ $('#monto').val(0); $('#SUBTOTAL').val(0);}  
}
	...
	return accounting.unformat(v, SysSepDecimales);
		...
		function Calcula_Porcentaje(){
		  if (document.frmOportunidad.monto.value>''){ 
		    ...
		    comision_monto =FormatFloatCharacter(document.frmOportunidad.comision_monto.value,DecimalSeparador);
		    monto= FormatFloatCharacter(document.frmOportunidad.monto.value,DecimalSeparador);
		...
		function GuardarOportunidad(){
		  ...
		  setTimeout(function() {
		    ...
		    $('#monto').removeAttr('disabled');
		  	...
		  	if(tiene_productos != 1){
			      var Validar_monto = $("#monto").val(); 
			      var formateado_monto=sinFormato(Validar_monto);
			      $("#monto").val(formateado_monto);
			      var Validar_com = $("#comision_monto").val(); 
			      var formateado_com=sinFormato(Validar_com);
			      $("#comision_monto").val(formateado_com);
			      var Validar_comporc = $("#comision").val(); 
			      var formateado_comporc=sinFormato(Validar_comporc);
