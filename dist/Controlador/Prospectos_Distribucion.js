...
//	var templateOpcion = '<option value="{{TIPO}}" data-id="{{IDTIPOFILTRO}}" data-cat="{{CATE}}">{{FILTRO}}</option>';
	...
	//jsonFiltros   = _.union(jsonFiltros,jsonFiltros2);
	...
	SalesUp.Variables.ActivaMostrarFiltros = function(Op){
		...
		//Importante
		var arrayTablas ='1' ;//idComp.split(',');
...
SalesUp.Variables.Origen= function(Op){
...
SalesUp.Variables.MuestraTiempo=function(value){

	var origen = value;
	
	 if(origen < 3){
		$('#divEntidad').removeClass('w40').addClass('w50');
		$('#divOrigen').removeClass('w40').addClass('w50');	 
		$('#divTiempo').removeClass('Mostrar').addClass('Ocultar');
	 }
	 else{
		$('#divEntidad').removeClass('w50').addClass('w40');
		$('#divOrigen').removeClass('w50').addClass('w40');	 
		$('#divTiempo').removeClass('Ocultar').addClass('Mostrar');
	 }
