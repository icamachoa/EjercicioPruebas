...
SalesUp.Variables.valorMonedaActual;
		...
		$("#BtnAceptar").click(function(e){
			...
			if(SalesUp.Variables.estaVisibleCotizador && SalesUp.Variables.idDocumento){
				...
				if(!AccionDocumento){SalesUp.Variables.Pregunta();			}
	
				else{ 	 SalesUp.Variables.GuardarConOpciones(AccionDocumento);}
			}
			else{
          ...
          setTimeout(function(){ $('#seguimiento').focus(); },1000);
			...
			$('#monto').val(jsonInfoProspecto[0].MONTO);
			$('#comision_monto').val(jsonInfoProspecto[0].COMISION_MONTO);
			$('#comision').val(jsonInfoProspecto[0].COMISION);
			...
			var j = jsonInfoProspecto ;//SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/jsonDatosOportunidad.dbsp', Parametros:'idoportunidad='+IdOportunidad, DataType:'json'}).jsonDatos;
			for(var i=0; i<=64; i++){
				var dato = 'CAMPO'+i+'O';
				var $d = $('#CO'+i);
				var info = j[0][dato];
        ...
        function Calcula_Comision(){
          EstandarizaNumero (document.frmOportunidad.monto);
          if (document.frmOportunidad.monto.value>''){
            ...
            document.frmOportunidad.comision_monto.value = roundNumber (aCom,2);
          }
        }/*Calcula_Comision*/
        ...
        function Calcula_Porcentaje(){
          if (document.frmOportunidad.monto.value>''){
            ...
            comision_monto =FormatFloatCharacter(document.frmOportunidad.comision_monto.value,SP_DECIMALSEPARATOR);
            monto= FormatFloatCharacter(document.frmOportunidad.monto.value,SP_DECIMALSEPARATOR);
            var tmp=((comision_monto/monto)*100);
			if (isNaN(tmp))       tmp = 0;
			if (tmp=='Infinity') tmp = 0;
            document.frmOportunidad.comision.value=redondeo2decimales(tmp);
          }
        } /*Calcula_Porcentaje*/
        ...
        SalesUp.Variables.Guardar = function(){
         ...
         var Pasa = false;
          ...
          var montoF =  $('#monto').val();          
          document.getElementById("monto").value = montoF;
          ...
          }else{ $('#BtnAceptar').removeAttr('disabled'); SalesUp.Sistema.OcultarOverlay(); }
...
SalesUp.Variables.CalculaMontoPorMoneda = function(){
	...
	var nuevoMonto = parseFloat(SalesUp.Sistema.calculaMontoTipoCambio({tipoCambioAnterior:SalesUp.Variables.tipoDeCambioActual,nuevoTipoCambio:valorMoneda,monto:montoPrevio}));
...
SalesUp.Variables.Pregunta  = function(){
		...
		$('#CrearReemplzar').attr('onclick', 'SalesUp.Construye.CierraAlerta({Elemento:this  ,Callback:SalesUp.Variables.GuardarConOpciones(2)  });');
}
...
$('#AccionDocumento').val(tipoAccionDocumento);
					...
					if(SalesUp.Variables.estaVisibleCotizador){	
							...
							SalesUp.Variables.ConstruyeJsonCotizador({t:$('#CapturaProdutos #BtnAceptar')});
					}
