...
//Nuevo Modulo
var fecha                                       = new Date();
var fecha_ini                                   = fecha.toLocaleDateString();
fecha.setDate(parseInt(fecha.getDate()) + 1);
var fecha_fin = fecha.toLocaleDateString();
...
var complementoTemplate = '<td ><input type="text" id="fecha_ini{{IDTR}}" value="{{Fecha_ini}}" class="BoxSizing InfoData Fecha InputCantidad w100 Pointer" /></td>' 
						+ '<td ><input type="text" id="fecha_fin{{IDTR}}" value="{{Fecha_fin}}" class="BoxSizing InfoData Fecha InputCantidad w100 Pointer" /></td>' 
						+ '<td id="noche{{IDTR}}" class="Noche tCen">{{Noches}}</td>';


//Nuevo Modulo

var templateArt = '{{#each Articulo}}<tr class="zebra" data-IDTR="{{IDTR}}" id="TR{{IDTR}}" data-desc="{{CANTIDAD_DESC}}" data-porc="{{PORCENTAJE_DESC}}" data-observaciones="{{COMENTARIO}}">' 
				+ '<td  style="text-align: center; width: 40px;"><span  tip="Arrastrar para ordenar" class="sortear Tip2 Pointer"><i class="fa fa-lg  fa-bars"></i></span><span class="Pointer Tip2 fa fa-lg fa-trash-o" tip="Eliminar articulo" onclick="SalesUp.Variables.EliminaArticulo(this)"></span>' 
				+ '<span original-title="" class="DescuentoProducto Pointer Tip2 fa fa-lg fa-hand-scissors-o" tip="Aplicar descuento" onclick="SalesUp.Variables.CreaDescuentoPorProducto({t:this})"></span></td>' 
				...
				+ '<td  style="text-align: center; width:45px">{{Unidad}}</td>' 
				+ '<td  style="text-align: center; width:45px">{{Existencia}}</td>' 
				...
				+ '<td  style="text-align: right"><div class="w20"><span data-IDTR="{{IDTR}}" data-Margen="{{Margen}}" data-idProducto="{{idProducto}}" data-PctNuevoMargen="{{PctNuevoMargen}}" data-PrecioLista="{{PrecioLista}}" data-PrecioUsuario="{{PrecioSinFormato}}" data-costo="{{Costo}}" data-precioMinimo={{PrecioMinimo}} class="Pointer Tip8 alertaW" tip="Análisis de precio" onclick="SalesUp.Variables.MuestraWarning({t:this})"><i class="fa fa-lg fa-exclamation-triangle"></i></span></div>' 
				+ '<div class="w80"><span data-precio={{PrecioSinFormato}} data-comision="{{Comision}}" data-cantidad={{Cantidad}} data-precioBase ={{PrecioSinFormato}} data-impuesto="{{JsonImpuesto}}" data-noche="{{Noches}}" class="subtotal">{{Subtotal}}</span></div></td>' 
				+ '</tr>{{/each}}';
	...
	if (IDTR != 0) {
		if (idMonedaEmpresaPrecio != idMonedaEmpresaGlobal) {
			...
			if (SalesUp.Variables.permisoCambiarMoneda) {
				...
				var monto = parseFloat($('#precio').val());

				$('#precio').val(SalesUp.Variables.ConversorMoneda(tipoCambio, SalesUp.Variables.valorMonedaActualCotizador, monto));
...
SalesUp.Variables.AgregarArticulo = function() {
	...
	if (SalesUp.Valida.ValidaObligatorios({
        ...
        var jsonNuevoArticulo = '{"Articulo": [{' 
        						+ '"Codigo":"' + Codigo + '",'
        						+ '"idProducto":"' + idProducto+'",'
        						+ '"IDTR":"' + IDTR + '",' 
        						+ '"Articulo":"' + Articulo + '",' 
        						+ '"Clase":"' + Clase + '",'
        						+ '"PctNuevoMargen":"' + PctNuevoMargen + '",'
        						+ '"Margen":"' + Margen + '",' 
        						+ '"PrecioLista":"' + PrecioLista + '",' 
        						+ '"Unidad":"' + Unidad + '",'
        						+ '"Existencia":"'+ Existencia + '",' 
        						+ '"SrcImagenes":"' + srcImagenes + '",' 
        						+ '"Cantidad":' + Cantidad + ',' 
        						+ '"ImpuestoMonto":"' + SalesUp.Variables.MonedaSimbolo(impuestoMonto) + '",' 
        						...
        						+ '"TIPMONEDA":"' + monedaTip + '",' 
        						+ '"IDMONEDA":"' + idMoneda + '",' 
        						+ '"Precio":"' + SalesUp.Variables.MonedaSimbolo(Precio) + '",' 
        						+ '"Descuento":"' + SalesUp.Variables.MonedaSimbolo(Op[0].CANTIDAD_DESC) + '",' 
        						+ '"PrecioSinFormato"  :"' + Precio + '",' 
        						+ '"readonly":"' + readonly + '",' 
        						+ '"NombrePrecio" :"' + $('#NombrePrecio').val() + '",' 
        						+ '"Fecha_ini":"' + $('#fecha_ini').val() + '",' 
        						+ '"Fecha_fin":"' + $('#fecha_fin').val() + '",' 
        						+ '"Noches":"' + calculo_noche + '",' 
        						+ '"CANTIDAD_DESC":"' + Op[0].CANTIDAD_DESC + '",' 
        						+ '"PORCENTAJE_DESC":"' + parseFloat(Op[0].PORCENTAJE_DESC*100) + '",' 
        						+ '"COMENTARIO":"' + Op[0].COMENTARIO + '",' 
        						+ '"Subtotal":"' + SalesUp.Variables.MonedaSimbolo(Subtotal) + '"}]}';

        jsonNuevoArticulo = jsonNuevoArticulo.replace('\\', '');
		var jsonArticulo = JSON.parse(jsonNuevoArticulo);
...
SalesUp.Variables.ActualizaMoneda = function() {
	var nuevoTipoCambio = parseFloat($('#monedas option:selected').attr('data-tipoCambio'));
...
SalesUp.Variables.CambiaMoneda = function() {
	var nuevoTipoCambio = parseFloat($('#monedaCotizacion option:selected').attr('data-tipoCambio'));
	idMonedaString = $('#monedaCotizacion option:selected').text();
	var idMonedaempresa = $('#monedas').val();
...
SalesUp.Variables.RecalculaImpuestos = function() {
	...
	$('#comision_monto').val(SalesUp.Variables.roundDos(importeLocalComisionTotal));
}
...
SalesUp.Variables.ImpuestosProductosDIV = function() {
	...
	var templateImpuestos = '{{#each Impuestos}}<div class="w100">' + '	<div style="float: right;margin-top: 2px;" class="w30">' + '		<span  class="w50" style="float: left; text-align:right;  margin-left: 32px;"><b>{{nombre}} (' + idMonedaString + '):</b></span>' + '		<span id="impuesto{{indice}}"  data-monto="0" style="float: right;">$0.00</span>' + '	</div>' + '</div>{{/each}}';
...
SalesUp.Variables.AgregarArticuloBD = function(IDOPORTUNIDAD, idProspecto, tko, idArchivo) {
	...
	for (var i = 0; i < SalesUp.Variables.jsonProductosBD.length; i++) {
		...
		if (i == 0) {
			tipoCambio = parseInt(SalesUp.Variables.jsonProductosBD[i]['TIPOCAMBIO']);
...
SalesUp.Variables.AgregaFuncionesInputs = function() {
	...
	$('.precio').attr('onchange', 'SalesUp.Variables.CalculaSubtotal({t:this}, 2);').attr('onblur', 'SalesUp.Variables.CalculaSubtotal({t:this}, 2);').attr('onfocus', 'SalesUp.Variables.QuitaFormato({t:this}, 1);').attr('onKeyPress', 'return SalesUp.Valida.valDecimales({e:event, t:this, v:value, Cerca: true, DestinoMsj:\'#popup-contenedor\'})').attr('onkeyup', '$(this).removeClass("DatoMal")');
	$('.precioWarning').attr('onchange', 'SalesUp.Variables.ValidaPrecioMinimo({t:this}, 2);').attr('onblur', 'SalesUp.Variables.RegresaFormato({t:this},1);').attr('onfocus', 'SalesUp.Variables.QuitaFormato({t:this}, 1);').attr('onKeyPress', 'return SalesUp.Valida.valDecimales({e:event, t:this, v:value, Cerca: true, DestinoMsj:\'#popup-contenedor\'})').attr('onkeyup', '$(this).removeClass("DatoMal"); SalesUp.Variables.CalculaWarning({t:this});');
}
...
SalesUp.Variables.CreaDescuento = function() {
	...
	var Html = '	<div style="margin-top:5px; text-align: left" class="w100 TitDiv">' + '    <i class=""></i>' + '    Aplicar descuento general' + ' 	</div>' + '  	<div class="clear"></div><form id="descuentoForm">' + '  	<div class="w100">' + '	  	<div class="w100">' + '			<div class="BoxInfo w50">' + '				<label tip="Descripcíon del descuento" class="BoxSizing InfoLabel Tip4">Descuento</label>' + '				<input type="text" class="BoxSizing InfoData InfoObligatorio" id="cantidadDesc" name="cantidadDesc" onKeyUp="SalesUp.Variables.CalculaDescuentoInformacion(1,1);" onChange="SalesUp.Variables.CalculaDescuentoInformacionValores(1,1);" onfocus="SalesUp.Variables.QuitaFormato({t:this}, 1);" onblur="SalesUp.Variables.RegresaFormato({t:this});">' + '	  		</div>' + '	 		<div class="BoxInfo w50">' + '				<label tip="Porcentaje del descuento" class="BoxSizing  InfoLabel Tip4">Porcentaje</label>' + '				<input type="text" class="InfoData BoxSizing InfoObligatorio" id="porcentajeDesc" name="porcentajeDesc" onKeyUp="SalesUp.Variables.CalculaDescuentoInformacion(2,1);" onChange="SalesUp.Variables.CalculaDescuentoInformacionValores(2,1);"> <span class="pimpuesto">%</span>' + '	  		</div>' + '	  	</div>' + '		<div class="clear"></div>' + '			<span class="DescuentoCotizador BoxSizing" style="font-style: italic !important; font-weight:normal !important"  id="textoDesc">Con este descuento está cediendo el <span id="margenDesc" style="font-weight: bold;">0%</span> de su margen posible en esta operacion</span>' + '	</div></form>';
...
SalesUp.Variables.desdeTabla = true;
	...
	if (SalesUp.Valida.ValidaObligatorios({
		...
		var nuevoTipoCambio = parseFloat($('#modificaCambio').val());
...
SalesUp.Variables.PopCambiaMoneda = function(idMoneda, monedaTip, tipoCambio, idMonedaCotizacion) {
	...
	$('#monedaCotizacion').html(SalesUp.Variables.htmlMonedas);
	$('#monedaCotizacion').val(SalesUp.Variables.IdEmpresaMoneda);

	if (SalesUp.Variables.permisoCambiarMoneda) {
		$('#modificaCambio').val(SalesUp.Variables.MonedaSimbolo(SalesUp.Variables.valorMonedaActualCotizador));
	} else {
		$('#modificaCambio').val(SalesUp.Variables.valorMonedaActualCotizador);
	}

	setTimeout(function(){
			if(SalesUp.Variables.valorMonedaActualCotizador==0)
				SalesUp.Variables.valorMonedaActualCotizador = parseFloat($('#monedaCotizacion option:selected').attr('data-tipoCambio'));
...
SalesUp.Variables.ActualizaMonedaDesdePop = function() {
	...
	if (SalesUp.Variables.permisoCambiarMoneda) {
		$('#modificaCambio').val(SalesUp.Variables.MonedaSimbolo(nuevoTipoCambio));
	} else {
		$('#modificaCambio').val(nuevoTipoCambio);
	}
...
SalesUp.Variables.CreaDescuentoPorProducto = function(Op) {
	...
	var cantidad_descuento     = $($padre).attr('data-desc');
	var porcentaje_desc        = $($padre).attr('data-porc');
	...
	var Html = '	<div style="margin-top:5px; text-align: left" class="w100 TitDiv">' + '    <i class=""></i>' + '    Agregar descuento o comentario' + ' 	</div>' + '  <div class="clear"></div>' + '<form id="descuentoFormProducto">' + '	<input type="hidden" id="tr" name="tr" value="' + id + '" />' + '		<div class="w100 tIzq" style="padding-bottom:5px">' + '			<span class="Ellipsis">' + NombreProducto + '</span>' + '		</div>' + '	  	<div class="w100">' + '			<div class="BoxInfo w50">' + '				<label tip="Descripcíon del descuento" class="BoxSizing InfoLabel Tip4">Descuento</label>' + '				<input type="text" class="BoxSizing InfoData InfoObligatorio" id="cantidadDescP" name="cantidadDescP" data-IDTR="' + id + '" onKeyUp="SalesUp.Variables.CalculaMargenDescuento({t:this}, 1);"  onfocus="SalesUp.Variables.QuitaFormato({t:this}, 1);" onblur="SalesUp.Variables.RegresaFormato({t:this});">' + '	  		</div>' + '	 		<div class="BoxInfo w50">' + '				<label tip="Porcentaje del descuento" class="BoxSizing  InfoLabel Tip4">Porcentaje</label>' + '				<input type="text" class="InfoData BoxSizing InfoObligatorio" id="porcentajeDescP" data-IDTR="' + id + '" onKeyUp="SalesUp.Variables.CalculaMargenDescuento({t:this}, 2);"> <span class="pimpuesto">%</span>' + '	  		</div>' + '	  	</div>' + '		<div class="clear"></div>' + '			<span class="DescuentoCotizador BoxSizing" style="font-style: italic !important; font-weight:normal !important"  id="textoDesc">Con este descuento está cediendo el <span id="margenDescProducto" style="font-weight: bold;">0%</span> de su margen posible en esta operacion</span>' + '		<div class="BoxInfo w100 BoxInfoTextArea BoxSizing ">' + '			<label tip="Agregue observaciones del producto" class="BoxSizing InfoLabel Tip4" original-title="">Observaciones</label>' + '			<textarea  maxlength="" id="observaciones" name="observaciones" class="TextAreaData BoxSizing">' + observaciones + '</textarea>' + '		</div>' + '</form>';
...
SalesUp.Variables.AceptaDescuentoProducto = function(Op) {
	...
	if (SalesUp.Valida.ValidaObligatorios({
		...
		var cantidad_descuento = $('#cantidadDescP').val();
		var porcentaje_desc    = $('#porcentajeDescP').val();
...
SalesUp.Variables.CalculaMargenDescuento = function(Op, tipo) {
	...
	if (tipo == 1) {
		...
		porcentajeDesc = parseFloat($($t).val());
		if (!isNaN(porcentajeDesc)) {
