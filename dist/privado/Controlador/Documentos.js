...
SalesUp.Variables.GuardaNombreCarpeta = function(Op){
  ...
  if(Editar){
    ...
    var Existe =  _.filter(SalesUp.Variables.TodosArchivos, function(j){
        if(($.trim(j.Descripcion).toLowerCase() === $.trim(NombreCarpeta).toLowerCase())&&(j.IdPadre == SalesUp.Variables.CarpetaActual)&&(j.IdCarpeta!=idCarpeta)&&(SalesUp.Variables.TipoCarpeta == j.TipoCarpeta)){return j;}
    });
    ...
    var Existe =  _.filter(SalesUp.Variables.TodosArchivos, function(j){
      if(($.trim(j.Descripcion).toLowerCase() === $.trim(NombreCarpeta).toLowerCase())&&(j.IdPadre == SalesUp.Variables.CarpetaActual)&&(SalesUp.Variables.TipoCarpeta == j.TipoCarpeta)){return j;}
    });
  ...
  var tArchivo = TipoCarpeta;
  if (tArchivo == '') {
    tArchivo = SalesUp.Variables.TipoCarpeta;
  };

  SalesUp.Variables.CargaListaArchivos({IdCarpeta:SalesUp.Variables.CarpetaActual, TipoArchivo:tArchivo});
