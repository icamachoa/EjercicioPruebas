...
SalesUp.Variables.ReCalCularComisiones=function(){
  ...
  var   noPagados=$('.corto1a').not('.colorpagado');
        ultimoPago=SalesUp.Sistema.formatoNumero(ultimoPago);
        pagoscomision=SalesUp.Sistema.formatoNumero(pagoscomision);
  //prorrateadas 
  if(tipo=='0'){
    for(var i=0; i<=noPagados.length-1; i++){
        var a=$(noPagados[i]).attr('id');
        if(i==(noPagados.length-1)){
            $('#'+a).val(ultimoPago);
        }else{
          $('#'+a).val(pagoscomision);
        }
    ...
    var id=$(noPagados[0]).attr('id');
    for(var r=0; r<=noPagados.length-1; r++){
      var id= $(noPagados[r]).attr('id');
      if(r==0){
...
SalesUp.Variables.CalcularFecha=function(){
  ...
  for(var i=r; i<=xx.length-1; i++){
    ...
    if(tipo=='6'){
      ...
      var x=f.add(incrementa,t).format(Formato); 
      $('#fecha'+a).val(x);
