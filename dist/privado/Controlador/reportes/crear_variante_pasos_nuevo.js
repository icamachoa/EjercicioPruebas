...
Handlebars.registerHelper('tieneUniversoAnterior', function(paso,controlSelect) {
  	...
  	if (paso == 1) {
  		...
  		out += '<select class="Tip2 UniversoPaso" data-paso="'+paso+'" Tip="Paso anterior" id="UniversoPaso'+paso+'" name="UniversoPaso'+paso+'" onchange=" SalesUp.reportes.crearvariantespasos.CambiaLabelPaso({universo:value,paso:'+paso+'});" style="width:12%;">';
...
var reporteVariantesPasos = function(){
	...
	this.CambiaLabelPaso = function(_obj){
	...
	this.LimipiarLocalStorageFiltros = function(){
	...
	this.AgrupacionYperiodicidad = function(Op){
	...
	this.CiertosGrupos = function(Op){
		...
		var template = ' <label class="InfoLabel LabelGrupo Pointer" data-activo="0"><input id="Checkbox_Grupo_{{IdGrupo}}" type="checkbox" onchange=" SalesUp.reportes.crearvariantespasos.SeleccionarGrupo({Valor:{{IdGrupo}}, Elemento:this })"/><i class="fa fa-square"></i><i class="fa fa-check-square"></i> {{Grupo}}</label>';
		SalesUp.Construye.ReemplazaTemplate({Destino:'#CiertosGrupos', Datos:jsonGrupos , Template:template });
	}

	this.CompartirCon = function(Op){
		var Valor = Op.Valor;
		$('#CiertosGrupos').slideUp();
		$('#CompartirCon').val(Valor);
		if(Valor==3){  SalesUp.reportes.crearvariantespasos.CiertosGrupos(); $('#CiertosGrupos').slideDown();$('#CompartirCon').val('');}
	}

	this.SeleccionarGrupo = function(Op){
	...
	this.AgregarPaso = function(_Paso){
		var Paso = _.size($('.PasoBox'))+1;

		SalesUp.Sistema.BorrarItemDeAlmacen('TemplateAgregarPaso');
		var TemplateAgregarPaso = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateAgregarPasoVariante.dbsp', Almacen:'TemplateAgregarPaso'});
		var Dato={};
		Dato.Paso = Paso;
		...
		$('#NombrePaso'+Paso).focus();
		 ...
		 SalesUp.reportes.crearvariantespasos.EliminarPaso(Paso);

		if(Paso==15){$('#BtnAgregarPaso').hide();return false;}
		
	};

	this.CambiaPaso = function(Paso){
		...
		for (var i = 0; i < $padre.length; i++) {
			...
			$('#UniversoPaso' + pasoActual).attr('id','UniversoPaso'+pasoNuevo).attr('name','UniversoPaso'+pasoNuevo).attr('onchange',' SalesUp.reportes.crearvariantespasos.CambiaLabelPaso({universo:value,paso:'+pasoNuevo+'});').attr('data-paso',pasoNuevo);
			$('#botonFiltrar' + pasoActual).attr('id','botonFiltrar'+pasoNuevo).attr('onclick',' SalesUp.reportes.crearvariantespasos.ActivaMostrarFiltros({Paso:'+pasoNuevo+', Out:true});');
			...
			$('#botonCerrar' + pasoActual).attr('id','botonCerrar'+pasoNuevo).attr('onclick',' SalesUp.reportes.crearvariantespasos.VerOpcionesFiltros({Paso:'+pasoNuevo+', In:true});');
			$('#botonAceptar' + pasoActual).attr('id','botonAceptar'+pasoNuevo).attr('onclick',' SalesUp.reportes.crearvariantespasos.VerOpcionesFiltros({Paso:'+pasoNuevo+', In:true});');
			$('#FiltroTipoPaso' + pasoActual).attr('id','FiltroTipoPaso'+pasoNuevo).attr('name','FiltroTipoPaso'+pasoNuevo).attr('onchange',' SalesUp.reportes.crearvariantespasos.MostrarFiltro({Elemento:this, Filtro:value, Paso:'+pasoNuevo+' });')
			$('#OpcionesTipoFiltros' + pasoActual).attr('id','OpcionesTipoFiltros'+pasoNuevo).attr('name','OpcionesTipoFiltros'+pasoNuevo).attr('onchange',' SalesUp.reportes.crearvariantespasos.SeleccionarFiltro({Elemento:this, Filtro:value, Paso:'+pasoNuevo+' });')

			 SalesUp.reportes.crearvariantespasos.CambiaLabelPaso({paso:pasoActual,universo:universo});
		};

		$padre.val(universo);
	};

	this.EliminarPaso = function(Paso){
		...
		if(Paso>1 && tamanioPasos > 1){
			...
			$('#Paso'+Paso+' .BulletPaso').addClass('Pointer').mouseenter(function(){
				...
				var Paso = $(this).attr('data-Paso');
				 ...
				 SalesUp.reportes.crearvariantespasos .CambiaPaso(Paso);
		...
		var templateUniverso = '<span id="{{id}}" class="FiltroEtiqueta Universo" data-Paso="{{Paso}}" data-Cat="{{Cat}}" data-Tipo="{{Tipo}}">{{TextoFiltro}} <span class="ConfingFiltro Transition" onclick=" SalesUp.reportes.crearvariantespasos.ActivaOpcionesEtiqueta({Elemento:this, Id:\'{{id}}\'});" ><i class="fa fa-ellipsis-v"></i></span></span>';
		var templateFiltros = '<span id="{{id}}" class="FiltroEtiqueta FiltroNormal" data-Operador="{{Operador}}" data-Universo="{{Universo}}" data-Paso="{{Paso}}" data-Cat="{{Cat}}" data-Tipo="{{Tipo}}" data-valor="{{Valor}}">{{TextoFiltro}} {{ValorFiltro}}<span class="ConfingFiltro Transition" onclick=" SalesUp.reportes.crearvariantespasos.ActivaOpcionesEtiqueta({Elemento:this, Id:\'{{id}}\'});"><i class="fa fa-ellipsis-v"></i></span></span>';
	...
	this.ActivaMostrarFiltros = function(Op){
		...
		if((!HayUniversoProspectos)&&(!HayUniversoOportunidades)&(!HayUniversoClientes)&(!HayUniversoOportunidadesTransacciones)){
		 ...
		 SalesUp.reportes.crearvariantespasos.VerOpcionesFiltros({Paso:Paso, Out:Out});
	...
	this.MostrarFiltro = function(Op){
		...
		if(Categoria==0){
			...
			if(Existe==0){
				$FiltrosPaso.append(Etiqueta);
				 SalesUp.reportes.crearvariantespasos.VerOpcionesFiltros({Paso:Paso, In:true});
			}else{
		...
		if(Filtro!=''){
			$('#Load'+Paso).show();
			setTimeout(function(){
				(Filtro>0) ?  SalesUp.reportes.crearvariantespasos.CargaFiltrosSistema(Op) :  SalesUp.reportes.crearvariantespasos.CargaFiltrosPersonalizados(Op);
			}, 10);
		}
	...
	this.CargaFiltrosPersonalizados = function(Op){
		return true;
	}/*SalesUp.Variables.CargaFiltrosPersonalizados*/


	this.SeleccionaPasoAnterior = function(Paso){
	...
	this.SeleccionarFiltro = function(Op){
		...
		if(Naturaleza == ''){
			 SalesUp.reportes.crearvariantespasos.SeleccionaPasoAnterior(Paso);
		}
	...
	this.ActivaOpcionesEtiqueta = function(Op){
		...
		var MenuOpciones = '';
		(Hermanos>1) ? MenuOpciones += '<span onclick=" SalesUp.reportes.crearvariantespasos.OpcionesAcciones({Accion:2, Id:\''+Id+'\', Operador:2 });" class="OpcionAcciones Pointer"><i class="fa fa-lg fa-calculator"></i> Operador lógico "O" '+Operador_O+'</span>':'';
		(Hermanos>1) ? MenuOpciones += '<span onclick=" SalesUp.reportes.crearvariantespasos.OpcionesAcciones({Accion:2, Id:\''+Id+'\', Operador:1 });" class="OpcionAcciones Pointer"><i class="fa fa-lg fa-calculator"></i> Condicion lógico "Y" '+Operador_Y+'</span>':'';
		MenuOpciones += '<span onclick=" SalesUp.reportes.crearvariantespasos.OpcionesAcciones({Accion:1, Id:\''+Id+'\' });" class="OpcionAcciones Pointer"><i class="fa fa-lg fa-trash"></i> Eliminar filtro</span>';
	...
	this.OpcionesAcciones = function(Op){
		var Id = Op.Id;
		var $Etiqueta = $('#'+Id);
	...
	this.GuardarDatos = function(){
		if(idConfiguracion==0){
			 SalesUp.reportes.crearvariantespasos.GuardaConfiguracion();
		}else{
			 SalesUp.reportes.crearvariantespasos.EditarConfiguracion();
		}
	}

	this.GuardaConfiguracion = function(){
		/*NUEVO*/
		...
		setTimeout(function() {
			...
			var iReCon = jsonReporteConfiguracion[0].iReCon;
			 ...
			 SalesUp.reportes.crearvariantespasos.GuardaPasosyFiltros({iReCon:iReCon});					
		}, 10); /*timeout*/

	} /* SalesUp.Variables.GuardaConfiguracion*/

	this.EditarConfiguracion = function(){
		/*EDITAR*/
		SalesUp.Sistema.MuestraEspera('',4);
		setTimeout(function() {
			if(!SalesUp.Valida.ValidaObligatorios()){ $('#Overlay').remove(); return false;}
			...
			SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/qryEditarReporteConfiguracion.dbsp', Parametros:Configuracion});
			 SalesUp.reportes.crearvariantespasos.GuardaPasosyFiltros({iReCon:idConfiguracion});					
		}, 10); /*timeout*/

	} /* SalesUp.Variables.GuardaConfiguracion*/ 

	this.GuardaPasosyFiltros = function(Op){
		...
		self.parent. SalesUp.reportes.crearvariantespasos.CargaReporteDafult(iReCon);
	...
	this.ConfirmarEliminar = function(){
		SalesUp.Construye.MuestraAlerta({
			...
			Boton1:'Eliminar',
			Boton2:'Cancelar',
			Callback1:' SalesUp.reportes.crearvariantespasos.EliminarConfiguracionReporte()',
			Icono1:'<i class="fa fa-trash"></i>',
			Icono2:'<i class="fa fa-times"></i>'
		});
	}

	this.EliminarConfiguracionReporte = function(){
		...
		self.parent. SalesUp.reportes.crearvariantespasos.CargaReporteDafult();
		self.parent.tb_remove();
	}

	this.EditarConfiguracionReporte = function(){
		...
		if(jsonInformacion.AgruparPor==1){ SalesUp.reportes.crearvariantespasos.Agrupacion({Valor:1}); $('#Periodicidad').val(jsonInformacion.Periodicidad);}
		...
		if(conGrupos>0){
			$('#Compartir').val(3);
			 SalesUp.reportes.crearvariantespasos.CompartirCon({Valor:3});
			...
			for (var y = 0; y <= splitIdGrupos.length - 1; y++){
				...
				$this.prop('checked',true);
				 SalesUp.reportes.crearvariantespasos.SeleccionarGrupo({Valor:idGrupo, Elemento:$this });
			};
		}
		...
		for (var i = 0; i <= jsonPasos.length - 1; i++){
			...
			var Naturaleza = jsonPasos[i].Naturaleza;
			 ...
			 SalesUp.reportes.crearvariantespasos.AgregarPaso(Universo);
...
$(function(){
	...
	(idConfiguracion==0) ? $('#BtnEliminar').remove():'';
	 SalesUp.reportes.crearvariantespasos.LimipiarLocalStorageFiltros();
	self.parent.SalesUp.Sistema.CambiarTamanioPopUp({Alto:235});
	 SalesUp.reportes.crearvariantespasos.AgrupacionYperiodicidad();
	...
	if(idConfiguracion==0){
		 SalesUp.reportes.crearvariantespasos.AgregarPaso();
	}else{
		 SalesUp.reportes.crearvariantespasos.EditarConfiguracionReporte();
	}
