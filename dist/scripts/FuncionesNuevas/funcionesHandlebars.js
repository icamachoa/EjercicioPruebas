...
Handlebars.registerHelper('hlpDosCeros', function(numero){
  ...
  //str = SalesUp.Sistema.formatoNumero(str);
  return new Handlebars.SafeString(str);
});
...
Handlebars.registerHelper('hlpModuloActivo', function(modulo,options){

  if (typeof modulo != "number") {
    return options.fn(this);
  }else{
    if (SalesUp.Sistema.EstaActivoModulo({Modulo:modulo})) {
      return options.fn(this);
    }
    else{
      return options.inverse(this);
...
Handlebars.registerHelper('hlpEntidad',function(categoria){
  var str = '';
  switch (categoria){
    case 1:
      str = 'Contactos';
...
Handlebars.registerHelper('hlpRoundDos', function(numero){
  var str;
  str = (Math.round(numero * 100) / 100).toFixed(2);
  str = SalesUp.Sistema.formatoNumero(str);
  return new Handlebars.SafeString(str);
...
Handlebars.registerHelper('hlpColumnasReporteVentasVS',function(n,columna1,columna2){
  var html = '';
  for (var i = 0; i < n; i++) {
    html +='<th class="tCen">'+columna1+'</th>';
    html +='<th class="tCen">'+columna2+'</th>';
  };
  return new Handlebars.SafeString(html);
});

Handlebars.registerHelper('hlpReporteVentasCR',function(mes,simbolo){
  var texto = '';
  texto = '{{hlp_Simbolo_Moneda '+mes+' "'+simbolo+'" 0}}';
  return new Handlebars.SafeString(texto);
});

Handlebars.registerHelper('hlpPermisosActivos', function(nombrePermiso,valoresPermitidos,options){
  var arrVal = JSON.parse(valoresPermitidos);
  var Permitido = SalesUp.Sistema.PermisoActivo(nombrePermiso,arrVal);
    if (Permitido) {
      return options.fn(this);
    }
    else{
