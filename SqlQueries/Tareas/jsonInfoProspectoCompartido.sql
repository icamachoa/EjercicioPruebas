//[tkp|Text,idu|Text,idg|Text,session.idempresa|Untyped,session.db|Untyped,tku|Text,tkgrupo|Text,]
--SELECT
...
DECLARE @TBTEMP TABLE (Ids VARCHAR(MAX), Usuario VARCHAR(MAX), LoTiene INT)
...
IF @TKU != '' BEGIN SET @LTIDUSUARIOS = <#SESSION.DB/>.dbo.obtieneListaIdUsuarios(@TKU) END
IF @TKGRUPO != '' BEGIN SET @LTGRUPOS = <#SESSION.DB/>.dbo.obtieneListaIdGruposUsuarios(@TKGRUPO) END
...
BEGIN
	 ...
	 SELECT U.TKU, U.Nombre + ' ' +ISNULL(U.APELLIDOS, ''), COUNT(PA.IDPROSPECTO) FROM <#SESSION.DB/>.dbo.USUARIOS U 
	 LEFT JOIN <#SESSION.DB/>.dbo.PROSPECTOS_ASIGNADOS PA ON PA.IDPROSPECTO = @IDPROSPECTO AND U.IDUSUARIO = PA.IDUSUARIO 
	 WHERE U.IDUSUARIO = @IDUSUARIO
	 GROUP BY U.TKU, U.NOMBRE, U.APELLIDOS
	 
	 SET @N = @N + 1
END

SET @USUARIOSNOCOMPARTIDOS = ''
SELECT @USUARIOSNOCOMPARTIDOS = @USUARIOSNOCOMPARTIDOS + USUARIO + ',', @IdsNoCompartidos = @IdsNoCompartidos + Ids + ','  FROM @TBTEMP WHERE LOTIENE = 0 GROUP BY USUARIO, IDS
