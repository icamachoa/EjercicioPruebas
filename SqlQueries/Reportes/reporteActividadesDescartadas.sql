...
BEGIN
	...
	FROM 
	 ...
	 LEFT JOIN <#SESSION.DB/>.DBO.USUARIOS U   WITH(NOLOCK) ON U.IDUSUARIO = P.IDUSUARIO
	 LEFT JOIN <#SESSION.DB/>.DBO.USUARIOS_GRUPOS G   WITH(NOLOCK) ON U.IDGRUPO = G.IDUSUARIOGRUPO
	 LEFT JOIN <#SESSION.DB/>.DBO.PAISES PA   WITH(NOLOCK) on PA.IDPAIS = p.IDPAIS 
	 LEFT JOIN <#SESSION.DB/>.DBO.ESTADOS E   WITH(NOLOCK) on E.IDESTADO = p.IDESTADO AND E.IDPAIS = P.IDPAIS 
	 LEFT JOIN <#SESSION.DB/>.DBO.PROSPECTOS_ORIGENES PO   WITH(NOLOCK) on PO.IDORIGEN = p.IDORIGEN 
...
BEGIN
	...
	FROM 
	 ...
	 LEFT JOIN <#SESSION.DB/>.DBO.EMPRESAS_LINEAS_PRODUCTO F  WITH(NOLOCK) ON  O.IDLINEA_PRODUCTO = F.IDLINEA_PRODUCTO
	 LEFT JOIN <#SESSION.DB/>.DBO.USUARIOS U   WITH(NOLOCK) ON U.IDUSUARIO = O.IDUSUARIO
	 LEFT JOIN <#SESSION.DB/>.DBO.USUARIOS_GRUPOS G   WITH(NOLOCK) ON U.IDGRUPO = G.IDUSUARIOGRUPO
	 LEFT JOIN <#SESSION.DB/>.DBO.PAISES PA   WITH(NOLOCK) on PA.IDPAIS = p.IDPAIS 
	 LEFT JOIN <#SESSION.DB/>.DBO.ESTADOS E   WITH(NOLOCK) on E.IDESTADO = p.IDESTADO AND E.IDPAIS = P.IDPAIS 
	 LEFT JOIN <#SESSION.DB/>.DBO.PROSPECTOS_ORIGENES PO   WITH(NOLOCK) on PO.IDORIGEN = p.IDORIGEN 
