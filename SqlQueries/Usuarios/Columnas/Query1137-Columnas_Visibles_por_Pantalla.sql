...
LEFT JOIN <#SESSION.DB/>.dbo.EMPRESAS_CAMPOS EC ON UC.IDCAMPO = EC.IDCAMPO AND EC.IDEMPRESA = @IDEMPRESA AND (EC.TIPO IN (SELECT SPLITVALUE FROM dbo.Split(@VizualizarEn,',')) OR EC.COMPARTIR IN (SELECT SPLITVALUE FROM dbo.Split(@VizualizarEn,','))) AND EC.TIPO != 5
