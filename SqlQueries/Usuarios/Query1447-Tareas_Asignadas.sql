...
''Pointer EventoTareasAsignadas T''+CONVERT(VARCHAR(MAX),U.IDUSUARIO) AS className, SALESUP_CT.DBO.Fecha(T.FECHA_VENCIMIENTO) AS start, SALESUP_CT.DBO.Fecha(T.FECHA_VENCIMIENTO_REALIZADOR) AS startRealizador ,
...
JOIN SALESUP_CT.dbo.V_TAREAS_ESTADOS VT WITH(NOLOCK) ON VT.IDESTADO = T.IDESTADO 
