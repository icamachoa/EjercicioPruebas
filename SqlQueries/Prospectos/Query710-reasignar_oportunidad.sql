//[session.idempresa|Untyped,session.idusuario|Untyped,session.convertcode|Untyped,idusuario|Integer,listap|Text,comentario|Text,reasignar_oportunidad|Integer,session.db|Untyped,tko|Text,tku|Text]
--update
/*PROTEGIDO*/
...
DECLARE @REASIGNAR_OPORTUNIDAD INT
...
SET @REASIGNAR_OPORTUNIDAD = ISNULL(:REASIGNAR_OPORTUNIDAD,0)
SET @TKU = ISNULL(:TKU,'')
SET @TKO = ISNULL(:TKO,'')
...
IF @TKU != ''
BEGIN
	SET @IDUSUARIOV = SALESUP_CT.DBO.obtieneIdUsuario(@TKU)
END
