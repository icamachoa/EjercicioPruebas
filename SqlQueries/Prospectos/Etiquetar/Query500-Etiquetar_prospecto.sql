//[session.idempresa|Untyped,etiquetasiniciales|Text,lista|Text,idprospecto|Integer,tkp|Text,session.db|Untyped,session.idusuario|Untyped,session.convertcode|Untyped,tketiq|Integer]
--update
...
IF @TKETIQ != 0 BEGIN
  SET @LISTA_ANTERIOR = <#SESSION.DB/>.dbo.obtieneListaIdEtiquetas(@LISTA_ANTERIOR)
  SET @LISTA_NUEVA = <#SESSION.DB/>.dbo.obtieneListaIdEtiquetas(@LISTA_NUEVA)
END
