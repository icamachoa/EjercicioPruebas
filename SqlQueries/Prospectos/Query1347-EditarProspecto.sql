...
SET @TKETIQ = ISNULL(:TKETIQ, 0)
IF @TKETIQ != 0 BEGIN
	/* SET @ETIQUETAS = <#SESSION.DB/>.dbo.obtieneListaIdEtiquetas(@ETIQUETAS) */
	/* SET @LTETIQUETASNUEVA = <#SESSION.DB/>.dbo.obtieneListaIdEtiquetas(@LTETIQUETASNUEVA) */
	SET @LTETIQUETASORIGINAL = <#SESSION.DB/>.dbo.obtieneListaIdEtiquetas(@LTETIQUETASORIGINAL)
END
...
IF @AUTORIZACIONPENDIENTE  = 1
BEGIN
  DECLARE @IDDUENIO_COMP INT
  SELECT @IDDUENIO_COMP = IDUSUARIO FROM <#SESSION.DB/>.dbo.COMPANIAS WITH(NOLOCK) WHERE IDCOMPANIA = @IDCOMPANIA
  INSERT INTO SALESUP_CT.dbo.SQS WITH(ROWLOCK) (IDUSUARIO, ALERTA, VALOR)  VALUES (@IDDUENIO_COMP, 'autPendientes',1)
END
