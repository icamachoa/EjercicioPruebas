//[listap|Text,listanueva|Text,operacion|Integer,session.db|Untyped,session.idempresa|Untyped,session.idusuario|Untyped,session.convertcode|Untyped,tkpe|Text,tku|Integer]
--insert
...
IF @TKU != 0 BEGIN
  SET @LISTAUSUARIOS = <#SESSION.DB/>.dbo.obtieneListaIdUsuarios(@LISTAUSUARIOS)
END
