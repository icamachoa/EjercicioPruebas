...
SET @LISTAETIQUETAS = <#SESSION.DB/>.dbo.obtieneListaIdEtiquetas(@LISTAETIQUETAS)
...
BEGIN
 ...
 UPDATE <#SESSION.DB/>.DBO.PROSPECTOS SET ULTIMAMODIFICACION = GETDATE() WHERE  IDEMPRESA = <#SESSION.IDEMPRESA/> and IDPROSPECTO in (select splitvalue FROM <#SESSION.DB/>.dbo.Split(@LISTAPROSPECTOS, ',')  GROUP BY splitvalue)
