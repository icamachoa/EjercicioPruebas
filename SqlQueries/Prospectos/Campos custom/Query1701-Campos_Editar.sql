...
IF @NUEVAOPCION != ''
BEGIN
     SET @STRSQL = 'UPDATE <#SESSION.DB/>.dbo.PROSPECTOS SET CAMPO'+CAST(@INDICE AS VARCHAR(MAX))+' = '''+CAST(@NUEVAOPCION AS VARCHAR(MAX))+''', ULTIMAMODIFICACION = GETDATE() WHERE CAMPO'+CAST(@INDICE AS VARCHAR(MAX))+' = '''+CAST(@VIEJAOPCION AS VARCHAR(MAX))+''''
	 EXEC (@STRSQL)
