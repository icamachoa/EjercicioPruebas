...
SELECT <#SESSION.IDUSUARIO/>, 'notificacionesPush', count(*) * -1 FROM <#SESSION.DB/>.DBO.USUARIOS_ALERTAS WITH(NOLOCK)  WHERE 
IDUSUARIO=<#SESSION.IDUSUARIO/> AND NOTIFICADO = 0 
AND IDUSUARIOALERTA IN (SELECT ID FROM @TABLA WHERE TipoSuceso NOT IN (5, 14, 20, 25, 24, 28, 32, 53, 58, 62, 63) ) 
HAVING  count(*)  > 0
