...
select @METASALCANZADAS = COUNT(1)
...
SELECT @TICKETS_ALERTA = COUNT(1) FROM CONTROL.TICKETS.DBO.TICKETS T
OUTER APPLY(
...
SELECT @NOLEIDOS AS noLeidos, @IDTABINBOX as idTabInbox, @ORDEN AS orden, @FUERZACONTRASENA AS fuerzaPsw, 
@TICKETS AS Tickets, @TICKETS_ALERTA AS Tickets_Alertas, 1 AS Leido_Tickets_Alertas, 
@METASALCANZADAS AS metasAlcanzadas, @METASNOALCANZADAS AS metasNoAlcanzadas, @CONTEONOTIF AS notificaciones, 
@NOTIF_PUSH AS notificacionesPush, @REINICIARSESION  AS reiniciarSesion, @RECORDATORIO AS recordatorio,
@AUTPENDIENTES AS autPendientes
